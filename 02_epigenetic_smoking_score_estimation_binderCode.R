...
###########################################################################################################################
load("/net/PE1/raid1/yvonne/MARS_Smoking_Scores/bmiq_normized_marsenigma.rda")

src.data.dir.pre <- "/binder/mgp/datasets/2020_DexStim_Array_Human/methylation/10_final_qc_data/"
beta.mtrx.fn     <- "dex_methyl_bmiq_beta_mtrx.rds" 
beta.mtrx        <- readRDS(paste0(src.data.dir.pre, beta.mtrx.fn))

# in the script, they call beta matrix "normalized_MSet" =>
normalized_MSet <- beta.mtrx
...
missingcpgs <- cpgs %in% rownames(normalized_MSet)
...
#SABRE_data_down
select <- rownames(normalized_MSet) %in% Illig_data_down$cpgs
SABRE_normalisedbetas_down <- subset(normalized_MSet, select =="TRUE")


#SABRE_data_up
select <- rownames(normalized_MSet) %in% Illig_data_up$cpgs
SABRE_normalisedbetas_up <- subset(normalized_MSet, select =="TRUE")
...
#combine scores
############################################################################################################################################################
scores_combined <- as.data.frame(scores_up + scores_down)
rownames(scores_combined) = colnames(normalized_MSet)

summary(scores_combined)
